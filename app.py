...
@st.cache_data
def load_data():
    orders = pd.read_csv(DATA_DIR / "orders.csv", parse_dates=["due_date"])
    sched = pd.read_csv(DATA_DIR / "schedule.csv", parse_dates=["start", "end", "due_date"])
    return orders, sched
...
/* Tighten spacing and try to keep within one screen */
.block-container {{ padding-top: 4px; padding-bottom: 36px; padding-left: 8px; padding-right: 8px; max-width: 100%; }}
...
# ============================ TOP BAR =============================
# Minimal header to save vertical space (no big title, no filter toggle button)
st.write("")
...
# Apply filters to schedule (orders + wheels + machines)
...
# Filter schedule by wheel & machine
...
order_order = (
    orders[orders["order_id"].isin(sched["order_id"].unique())]
...
# Sort operations by machine + start
...
def _num_token_to_float(tok: str):
    """
    Accept both digits ("1.5", "2", "3,5") and words ("one", "two", "half") -> float
    """
    tok = tok.strip().lower()
    word_map = {
        "zero": 0,
...
def _parse_time_delta(text: str) -> dict:
    """
    Parse phrases like "2 days", "3 hours", "1.5h", "half a day" into dict of days/hours/minutes
    """
...
Examples:
User: "Delay order O-1005 by 2 days"
JSON: {{
  "intent": "delay_order",
  "order_id": "O-1005",
  "delta_days": 2,
...
User: "Swap order O-1003 with O-1008 on the filler"
JSON: {{
  "intent": "swap_orders",
  "order_id": "O-1003",
  "order_id_2": "O-1008",
...
def extract_intent(user_text: str) -> dict:
    """
    Lightweight rule-based extraction with optional OpenAI fallback.
    """
    txt = user_text.strip()
    low = txt.lower()
    ...
    order_id = m_order.group(1).upper().replace(" ", "").replace("_", "-") if m_order else None
...
def validate_intent(payload: dict, orders_df: pd.DataFrame, schedule_df: pd.DataFrame):
    """
    Basic validation: check presence of order(s), intent, and some numeric sanity.
    """
    ...
    if intent == "delay_order":
        if payload.get("delta_days") is None and payload.get("delta_hours") is None:
            return False, "Need a delta (days/hours) for delay/advance."
...
def apply_swap(schedule_df: pd.DataFrame, order_id_1: str, order_id_2: str) -> pd.DataFrame:
    ...
    # Swap start/end while keeping machines the same
    ...
    try:
        ...
        if ok:
            if payload["intent"] == "delay_order":
                delta = timedelta(
                    days=payload.get("delta_days", 0),
                    hours=payload.get("delta_hours", 0),
                    minutes=payload.get("delta_minutes", 0),
                )
                ...
                st.success(f"Applied delay/advance on order {payload['order_id']} ({delta}).")
                ...
                st.success(f"Swapped orders {payload['order_id']} and {payload['order_id_2']}.")
